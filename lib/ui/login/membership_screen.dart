  ...
  Future<void> _checkEmailDuplicate() async {
    ...
    try {
      ...
      if (res != null && res.result) {
        _showSnackBar('사용 가능한 이메일입니다.', Colors.green.shade600);
        setState(() {
          _isEmailChecked = true;
        });
      } else {
        _showSnackBar('이미 사용 중인 이메일입니다.', Colors.redAccent);
        setState(() {
      ...
      setState(() => _isLoading = false);
      _showSnackBar('네트워크 오류가 발생했습니다.', Colors.redAccent);
    }
  }

  Future<void> _submitRegister() async {
    if (!_isEmailChecked) {
      _showSnackBar('이메일 중복 확인을 해주세요.', Colors.redAccent);
      return;
    }
    ...
    if (_formKey.currentState?.validate() ?? false) {
      ...
      try {
        ...
        if (res != null && res.resultCode == 200) {
          _showSnackBar('회원가입에 성공했습니다!', Colors.green.shade600);
          
          // Log in automatically by dispatching LoginSucceeded event
          if (mounted) {
          ...
          _showSnackBar('회원가입에 실패했습니다.', Colors.redAccent);
        }
      } catch (e) {
        setState(() => _isLoading = false);
                            ...
                            SizedBox(
                              ...
                              child: ElevatedButton(
                                ...
                                style: ElevatedButton.styleFrom(
                                  backgroundColor: _isEmailChecked ? Colors.green.shade600 : const Color(0xFFFF9100),
                                  foregroundColor: Colors.white,
                                  shape: RoundedRectangleBorder(
                                    borderRadius: BorderRadius.circular(16),
                                  ),
                                ),
                                child: Text(_isEmailChecked ? '확인완료' : '중복확인'),
                              ),
                            ),
                            const SizedBox(height: 16),
